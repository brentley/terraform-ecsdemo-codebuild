...
resource "aws_s3_bucket" "s3_backend" {
  ...
  acl    = "private"
  bucket = "${data.aws_caller_identity.current.account_id}-${var.namespace}-tf-state"
...
resource "aws_dynamodb_table" "terraform_state_lock" {
  ...
  tags {
    Purpose = "Terraform state lock for state in ${data.aws_caller_identity.current.account_id}-${data.aws_region.current.name}-${var.namespace}-terraform-state-store:${var.s3_key} "
  }
}
